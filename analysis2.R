...
# Lag columns 230 to 772 by 2 years
cols_to_lag <- 230:764 # Specify the column range

# Create lagged columns
pdata <- pdata %>%
  mutate(across(all_of(cols_to_lag), ~ lag(., n = 2), .names = "lag2_{col}"))%>%
  dplyr::select(-all_of(cols_to_lag))

# Return to data frame

lag_2_2025 <- as.data.frame(pdata) %>%
...
#Prepare the data 
data_2025 <- lag_2_2025 %>%
  filter(vyear==2025)

#The training data
lag_2_train <- lag_2_2025 %>%
  filter(vyear != 2025) %>%
...
train_columns <- colnames(lag_2_train)

#The test data
lag_2_test <- lag_2_2025 %>%
  filter(vyear!=2025)%>%
  filter(vyear==2021)%>%
...
id = lag_2_test[,c("vid","populist_voteshare")]
...
write.csv(predictions_base,"/Users/tracemiller/Documents/bocconi courses/foundation/assignment group/predictions_lag2.csv")
