...
lag_1_2025 <- lag_1_2025%>%
  ...
  mutate(vstate = as.factor(vstate))# Convert vid back to factor if needed
...
x_test <- as.matrix(lag_1_test[, c(51:312, 1:3)])
...
results <- run_models_and_select_best(
  ...
  y_test = y_test,
  alpha_values = seq(0.05, 0.95, by = 0.05)  # Specify alpha values for ElasticNet
)
...
id = lag_1_test[,c("vid","populist_voteshare")]
...
write.csv(predictions_base,"/Users/tracemiller/Documents/bocconi courses/foundation/assignment group/predictions_lag1d.csv")
