...
#pdata <- pdata %>%
#  mutate(across(all_of(cols_to_lag), ~ lag(., n = 0), .names = "lag1_{col}"))%>%
...
x <- as.matrix(lag_1_train[,c(51:320,1:3)])
...
x_test <- as.matrix(lag_1_test[, c(51:320, 1:3)])
...
write.csv(predictions_base,"/Users/tracemiller/Documents/bocconi courses/foundation/assignment group/predictions_lag0.csv")
